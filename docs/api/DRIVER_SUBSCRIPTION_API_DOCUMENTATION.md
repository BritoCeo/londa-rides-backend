...
```json
{
  ...
  "payment_method": "card|bank_transfer|mobile_money (required)",
...
- `payment_method`: Required, must be one of: `card`, `bank_transfer`, `mobile_money`
- `payment_token`: Optional, must be a string if provided

### Update Driver Subscription
- `auto_renew`: Optional, must be a boolean
- `payment_method`: Optional, must be one of: `card`, `bank_transfer`, `mobile_money`
